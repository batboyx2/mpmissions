...
while {_amount>0} do {
	...
	for "_x" from 1 to _amount do {
		...
		while {_relocate} do {
			...
			if ([_trigger,_pos] call BIS_fnc_inTrigger and !surfaceIsWater _pos and (getPos _altlogic select 2)==(getPosATL _altlogic select 2) and {_pos distance [getPos vehicle _x select 0,getPos vehicle _x select 1,0]<CLY_minspawndist} count (CLY_players)==0 and {[_x,_pos] call BIS_fnc_inTrigger} count CLY_nospawntriggers==0) then {_relocate=false};
		};
		//Debug marker
		if (CLY_debug) then {
