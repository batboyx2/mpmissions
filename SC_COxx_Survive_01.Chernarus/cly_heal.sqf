	...
	if (_target!=player) exitWith {
		...
		[_target] spawn {
			...
			if (!isPlayer _unit and _unit getVariable "CLY_healings">0) then {
				_action=[
					format ["%1Take bandage%2</t>",_structure,if (_unit getVariable "CLY_healings">1) then {"s"} else {""}],
					"cly_heal.sqf",
	...
	while {alive _unit and _unit==player} do {
		...
		if (_reset) then {
			...
			_healaction=_unit addAction [
				format ["%1Bandage wounds</t>",_structure],
				"cly_heal.sqf",
				"heal",	//Argument
				1.37,		//Priority
			...
			_interruptaction=_unit addAction [
				format ["%1Cancel bandaging</t>",_structure],
				"cly_heal.sqf",
				"interrupt",
				1.4,
				true,
		...
		} forEach (getPosATL _unit nearObjects ["Man",_nearestalivedist]);
		
		if (_nearestalive!=_nearestaliveactive or _nearestalivedamage!=damage _nearestaliveactive) then {
			_nearestaliveactive removeAction _healotheraction;
			_nearestaliveactive removeAction _giveaction;
		...
		if (!isNull _nearestalive and (_nearestalive!=_nearestaliveactive or _nearestalivedamage!=damage _nearestaliveactive)) then {
			_nearestaliveactive=_nearestalive;
			_nearestalivedamage=damage _nearestalive;
			_claw="";
			if (damage _nearestaliveactive>0.28) then {_claw="claw1.paa"};
			if (damage _nearestaliveactive>0.59) then {_claw="claw2.paa"};
			if (damage _nearestaliveactive>0.9) then {_claw="claw3.paa"};
			if (_claw!="") then {_claw=format [" <img image='%1'/>",_claw]};
			_healotheraction=_nearestaliveactive addAction [
				format ["%1Bandage %2%3</t>",_structure,name _nearestaliveactive,_claw],
				"cly_heal.sqf",
				"healother",
				1.38,
				true,
			...
			_giveaction=_nearestaliveactive addAction [
				format ["%1Give bandage to %2%3</t>",_structure,name _nearestaliveactive,_claw],
				"cly_heal.sqf",
				"give",
				1.39,
				true,
