...
if (isServer) then {
	{
		if (((_x getVariable "hasFace") == 0) && (str (side _x) == "GUER")) then {
			_num = floor random 3;
			_face = switch (_num) do {
				case 0: {"AsianHead_A3_01";};
				case 1: {"AsianHead_A3_02";};
				case 2: {"AsianHead_A3_03";};
			};
			[[_x, _face], "setFace", true, true] call BIS_fnc_MP;
			diag_log format["diagnose_%1",_x];
			_x setVariable ["hasFace", 1];
		};
	} forEach allUnits;
};